...
monthly <- cslice(time$cur, num=12, ratio=1, names=month.abb)

## Slice vectors into climatological windows
mdata <- slice(data, monthly)
mtime <- slice(time$cur, monthly)
...
xr <- c(0,365*2) + min(time$cur)
yr <- range(data)

plot(NA, xlim=xr, ylim=yr, xlab=time$cur@units, ylab=varname)
for(i in month.abb){
    points(mtime[[i]], mdata[[i]], col=col[i])
}
