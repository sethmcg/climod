...
The following example shows... HOW SLICE WORKS -- EDIT ME HERE
...
plot(NA, xlim=xr, ylim=yr, xlab=t@units, ylab=indata$cur@units, main=varname)
for(i in month.abb){
...
EDIT ME: So that's how slicing works.  This here is how we use slicing
to do the moving window thing, and also demonstrate why it matters.
...
dx <- slice(x, daily, outer=TRUE)

par(mfrow=c(3,1))

boxplot(mx, pch='.', boxwex=1, outline=FALSE, main="Monthly window")
boxplot(dx, pch='.', boxwex=1, outline=FALSE, main="30-day moving window",
        whisklty=0, boxlty=0, boxfill="gray", xlab="day of year")

daynorm <- lapply(dx, normalize)
...
year, except for leap years, which have 366 days.

Climate models, on the other hand, typically use idealized calendars.
Most of them use a 365-day, or 'noleap' calendar in which every year
is exactly 365 days long, with no leap years ever.  However, there are
also some climate models that use a 360-day calendar, composed of 12
days each exactly 30 days long.
