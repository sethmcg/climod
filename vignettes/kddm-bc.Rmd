...
We will operate on data from the window that correponds roughly to the
winter solstice.  (Note that this is at index 320 because the data
starts in december, not january.... wait, what?)

# FIGURE OUT AND EXPLAIN 320!
...
Now we construct a transfer function from the normalize data.  The
...
that the PDF of `cur` matches the PDF of `obs`.
...
Finally, we denormalize the data.  We want the mean and standard
deviation of the current period to match that of the observed data, so
we pass in the normalized observations.  Note that we want to do this
matching based on the entire 30-year period shared between obs and
cur; if we did it year-by-year, not only would the parameter estimates
be less certain, we would be forcing the model output to match the
interannual variation of the obs data, which would by unphysical and
wrong.

```{r, highlight=TRUE, fig.show="hold", fig.width=7, fig.height=7}
## adjust denormalization using stats from entire 30-year obs/cur periods
matchdata  <- normalize(unlist(bcdata$obs))
adjustdata <- normalize(unlist(bcdata$cur))
...
result$fut <- rapply(fixed$fut, denormalize, how="replace",
                     match=matchdata, adjust=adjustdata)

bplot(result)

```

# EXPLAIN DIFFERENCE IN PDFS OF OBS & CUR.  IS THAT RIGHT?
# DENORMALIZE ON CUR IS NOT RIGHT - WE LOSE ALL THE IAV
